...
const CLASS_TYPES = ['WEBINAR', 'WORKSHOP', 'MENTORSHIP']
const LEVEL_OPTIONS = ['GENERAL', 'VIP', 'SUPREMO', 'MASTER']
const EMPTY_FORM = { title: '', description: '', type: 'WEBINAR', minLevel: 'GENERAL', startsAt: '', endsAt: '', meetUrl: '' }
...
export default function SchedulePanel() {
  ...
  const load = useCallback(async () => {
    try {
      const { data } = await api.get('/events/admin/all?types=WEBINAR,WORKSHOP,MENTORSHIP')
  ...
  function openEdit(e: any) {
    setEditing(e)
    setForm({ title: e.title, description: e.description ?? '', type: e.type, minLevel: e.minLevel,
  ...
  async function save() {
    ...
    setSaving(true)
    try {
      const payload = { ...form, host: user?.displayName ?? 'Profesor' }
  ...
  async function remove(id: string) {
    Alert.alert('Eliminar', '¿Eliminar esta clase?', [
  ...
  return (
    <>
      <Stack.Screen options={{ title: 'Agenda de Clases', headerRight: () => (
      ...
      {loading ? <ActivityIndicator color={Colors.gold} style={{ marginTop: 40 }} /> : (
        <FlatList data={events} keyExtractor={e => e.id} contentContainerStyle={{ padding: 16, gap: 8 }}
          renderItem={({ item }) => (
            <View style={styles.card}>
              <View style={styles.cardInfo}>
                ...
                <Text style={styles.cardMeta}>{new Date(item.startsAt).toLocaleString('es-ES')} · {item.type}</Text>
              </View>
          ...
          ListEmptyComponent={<View style={styles.empty}><Text style={styles.emptyText}>Sin clases programadas</Text></View>}
        />
      )}
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalBox}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>{editing ? 'Editar clase' : 'Nueva clase'}</Text>
              <TouchableOpacity onPress={() => setModal(false)}><Ionicons name="close" size={22} color={Colors.textMuted} /></TouchableOpacity>
            </View>
            <ScrollView style={{ maxHeight: 420 }}>
              <TextInput style={styles.input} placeholder="Título" placeholderTextColor={Colors.textMuted} value={form.title} onChangeText={t => setForm(p => ({ ...p, title: t }))} />
              ...
              <TextInput style={styles.input} placeholder="URL Meet/Zoom" placeholderTextColor={Colors.textMuted} value={form.meetUrl} onChangeText={t => setForm(p => ({ ...p, meetUrl: t }))} />
              <Text style={styles.fieldLabel}>Tipo</Text>
              <View style={styles.optionRow}>
                {CLASS_TYPES.map(t => (
                  <TouchableOpacity key={t} style={[styles.optionBtn, form.type === t && styles.optionActive]} onPress={() => setForm(p => ({ ...p, type: t }))}>
                    <Text style={[styles.optionText, form.type === t && { color: Colors.background }]}>{t}</Text>
                  </TouchableOpacity>
                ))}
              </View>
